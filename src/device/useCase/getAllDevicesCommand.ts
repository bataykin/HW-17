  ...
  implements IQueryHandler<GetAllDevicesCommand>
{
  ...
  async execute(query: GetAllDevicesCommand): Promise<any> {
    ...
    try {
      extractedPayload = await this.jwtService.verify(refreshToken, {
        secret: jwtConstants.secret,
      });
      if (!extractedPayload || !extractedPayload.userId)
        throw new UnauthorizedException("user protyX");
    } catch (e) {
      throw new UnauthorizedException("TokenExpiredError: jwt expired");
    }
    const devs: DeviceEntity[] = await this.devRepo.getAllDevices(
