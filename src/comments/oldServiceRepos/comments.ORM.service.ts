...
export class CommentsORMService implements ACommentService {
  constructor(
    protected readonly commentsRepo: CommentsORMRepo,
  ...
  async findCommentByIdOnUserId(commentId: string, userId: any): Promise<any> {
    ...
    const UserReactionOnComment = await this.likesService.getCommentLikeInfo(
      userId,
      commentId,
    );
  ...
  async getCommentsByPost(
    ...
    const comments = await this.commentsRepo.getCommentsByPost(
      postId,
      skipSize,
      PageSize,
    );
  ...
  async setLikeStatus(
    ...
    return await this.likesService.addReactionToComment(
      userId,
      commentId,
      likeStatus,
    );
