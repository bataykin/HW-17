...
describe("HW-19-4 (e2e)", () => {
  ...
  for (let i = 0; i < counter; i++) {
    it("POST => /blogger/blogs/:blogId/posts - create $counter posts on blog0 by user0", () => {
      return request(app.getHttpServer())
  ...
  for (let i = 0; i < counter; i++) {
    it("(PUT => /comments/:commentId/like-status - put dislike on comment to post)", () => {
      return request(app.getHttpServer())
        .put(`/comments/${comments[i].id}/like-status`)
  ...
  it("GET -> /comments/:commentId - return comment on post0 with 1 dislikes from user0 authed", () => {
    return request(app.getHttpServer())
      ...
      .then((res) => {
        ...
        expect(res.body.likesInfo.myStatus).toBe(LikesEnum.Dislike);
      });
  });
});
